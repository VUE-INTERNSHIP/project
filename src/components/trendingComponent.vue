<template>
  <div class="trending" ref="reff" @mousedown="mouseDown" @mouseup="mouseUp" @mousemove="mouseMove">
    <h3>Trendings</h3>
    <div class="trending-img" ref="refs">
      <div v-for="(img, x) in trendings" class="divv" :key="x">
        ...
        <div class="booked">
          ...
          <span v-if="!img.isBookmarked" class="book"><img src="../assets/icon-bookmark-empty.svg"
              :alt="img.title" /></span>
        </div>
        <div class="imgs">
        ...
        <div class="msg">
          <p><span>{{ img.year }}</span><span v-if="img.category === 'TV Series'" class="span"><img class="svg"
                src="../assets/icon-category-tv.svg" alt="" />{{ img.category }}</span> <span
              v-if="img.category === 'Movie'" class="span"><img class="svg" src="../assets/icon-category-movie.svg"
                alt="" />{{ img.category }}</span> <span class="last">{{ img.rating }}</span></p>
          <p>{{ img.title }}</p>
        </div>
      </div>
    </div>
